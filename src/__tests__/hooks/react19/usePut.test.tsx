...
import { usePut } from '../../../hooks/react19/usePut';

describe('usePut (React 19)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('should call onSuccess callback on successful PUT', async () => {
    const mockData = { id: 1, title: 'Updated Todo' };
    const mockApi = vi.fn().mockResolvedValue(mockData);
    const onSuccess = vi.fn();

    const { result } = renderHook(() =>
      usePut(mockApi, { enabled: false, onSuccess })
    );
  ...
  it('should call onError callback on PUT error', async () => {
    const error = new Error('Update Failed');
    const mockApi = vi.fn().mockRejectedValue(error);
    const onError = vi.fn();

    const { result } = renderHook(() =>
      usePut(mockApi, { enabled: false, onError })
    );
  ...
  it('should disable caching by default for PUT requests', async () => {
    const mockApi = vi.fn().mockResolvedValue({ id: 1, title: 'Updated Todo' });
  ...
  it('should support multiple mutations', async () => {
    ...
    const { result } = renderHook(() => usePut(mockApi, { enabled: false }));
  ...
  it('should execute on mount when enabled is true', async () => {
    const mockApi = vi.fn().mockResolvedValue({ id: 1, title: 'Updated Todo' });
    const { result } = renderHook(() => usePut(mockApi, { enabled: true }));
